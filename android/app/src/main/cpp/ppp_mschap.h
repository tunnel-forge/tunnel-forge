...
static inline int ppp_mschapv2_parse_failure(const uint8_t *msg, size_t len, ppp_mschapv2_failure_info_t *out) {
  ...
  size_t i = 0;
  while (i < len) {
    while (i < len && (msg[i] == (uint8_t)' ' || msg[i] == (uint8_t)'\t'))
      i++;
    if (i + 2 > len)
      break;
    uint8_t key = msg[i++];
    if (i >= len || msg[i] != (uint8_t)'=') {
      while (i < len && msg[i] != (uint8_t)' ')
        i++;
      continue;
    }
    i++;
    if (key == (uint8_t)'E') {
      if (ppp_mschapv2_parse_uint_token(msg, len, &i, &out->error_code) == 0)
        out->has_error_code = 1;
    } else if (key == (uint8_t)'R') {
      if (ppp_mschapv2_parse_uint_token(msg, len, &i, &out->retry) == 0)
        out->has_retry = 1;
    } else if (key == (uint8_t)'V') {
      if (ppp_mschapv2_parse_uint_token(msg, len, &i, &out->version) == 0)
        out->has_version = 1;
